...
class DedalusRBC_Env(gym.Env):
    ...
    sim_metadata = {"Lx" : np.pi, "Lz" : 1.0, "Ra" : 1e5, "Pr" : 0.71, "Ni" : 100, "Nk" : 64, "DiscardTime" : 100}
    ...
    def _D3_init(self):
        ...
        # Ensure that self.fp is initialized
        if not hasattr(self, 'fp') or self.fp is None:
            raise RuntimeError("Error: self.fp (flow properties) was not initialized correctly in _D3_init.")
    ...
    def _D3_RBC_setup(self, seed): #all this bit downwards is the dedalus rayleigh bernard convection
